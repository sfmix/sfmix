...
Every log_agent host runs Grafana Alloy, whose embedded node_exporter
(prometheus.exporter.unix, roles/grafana_alloy) remote-writes into
Prometheus as job="node" with instance=host=<inventory_hostname>.
...
def linux_hosts_dashboard():
    ...
    stats = [
        ("Hosts reporting", 'count(node_uname_info)', "short", NEUTRAL,
         "Distinct hosts with node_exporter series in the last 5m."),
        ("Alloy agents without host metrics",
         'count(alloy_build_info unless on (host) node_uname_info) or vector(0)',
         "short", [{"color": GREEN, "value": None}, {"color": AMBER, "value": 1}],
         "Alloy is alive but not yet shipping job=\"node\" — usually an agent "
         "that has not been re-templated since the node collector was added."),
        ("Hosts CPU > 80%",
    ...
    p.append(_text(
        "**Source:** Alloy's embedded node_exporter on every `log_agent` host "
        "(roles/grafana_alloy), remote-written as `job=\"node\"`. Virtual NICs "
        "(veth/docker/tap/fw*) are excluded at collection; tmpfs/overlay and "
        "loop/zvol devices are filtered in the queries. Generated by "
...
def host_view_dashboard():
    ...
    p.append(_ts("Traffic by interface", [
        ...
        description="RX above the axis, TX below. veth/docker/tap/fw* interfaces "
                    "are excluded at collection (roles/grafana_alloy)."))
    ...
    p.append(_text(
        "**Source:** Alloy's embedded node_exporter on this host "
        "(`prometheus.exporter.unix`, roles/grafana_alloy), remote-written as "
        "`job=\"node\"` every 30s. Logs for the same host: use the **Host Logs** "
        "link above (pre-filled). Generated by gen_nms_dashboards.py — edits "
        "will be overwritten.",
        {"h": 3, "w": 24, "x": 0, "y": y}))
    y += 3
