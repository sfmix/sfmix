...
def datasource_uid(gurl, hdrs, ds_type):
    for ds in _get_json(f"{gurl}/api/datasources", hdrs):
        if ds["type"] == ds_type:
            return ds["uid"]
    raise RuntimeError(f"no {ds_type} datasource in Grafana")
...
def push(gurl, hdrs, folder_uid, dash, ds_uid, loki_uid=""):
    blob = json.dumps(dash)
    blob = blob.replace("${DS_PROMETHEUS}", ds_uid).replace("${DS_LOKI}", loki_uid)
...
def main():
    ...
    ds_uid = datasource_uid(gurl, hdrs, "prometheus")
    loki_uid = datasource_uid(gurl, hdrs, "loki")
