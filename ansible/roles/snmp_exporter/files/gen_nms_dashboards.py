...
def twin_dashboard(dev, ifnames):
    ...
    slug = dev["name"].replace(".", "-")
    return dashboard(
        f"sfmix-twin-{slug}",
        f"Switch Twin — {dev['name']}",
        p,
        description=(f'Digital twin of {dev["name"]} ({dev["manufacturer"]} '
                     f'{dev["model"]}, {dev["site"]}). Generated by '
...
def main():
    ...
    push(gurl, hdrs, folder, participant_dashboard(), ds_uid)
    ...
    for dev in sorted(netbox_devices(), key=lambda d: d["name"]):
        ...
        if not ifnames:
            ...
            continue
        push(gurl, hdrs, folder, twin_dashboard(dev, ifnames), ds_uid)
    return 0
