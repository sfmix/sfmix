...
fn command_to_match_string(command: &Command) -> String {
    ...
    let resource = match command.resource {
        ...
        Resource::NdEvents => return "show lan-events".to_string(),
        Resource::NdEventDetail => {
            return format!("show lan-events detail {}", command.target.as_deref().unwrap_or("*"));
        }
        Resource::NdEvidence => return "show lan-events evidence".to_string(),
